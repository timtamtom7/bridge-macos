    ...
    func fetchContacts(from device: Device) async -> [Contact] {
        ...
        for dbPath in possiblePaths {
            if FileManager.default.fileExists(atPath: dbPath) {
                return await readContactsFromDatabase(at: dbPath, deviceUDID: device.udid)
            }
        }
        return []
    }

    private func readContactsFromDatabase(at path: String, deviceUDID: String) async -> [Contact] {
        // Placeholder: full SQLite DB reading requires private APIs
        return []
    }
    ...
    nonisolated func fetchMacContacts() -> [Contact] {
        ...
        do {
            try CNContactStore().enumerateContacts(with: request) { cnContact, _ in
                let addrParts = cnContact.postalAddresses.map { addr -> String in
                    let v = addr.value
                    return "\(v.street), \(v.city)"
                }
                let contact = Contact(
    ...
    nonisolated func saveContactsToMac(_ contacts: [Contact]) {
        for contact in contacts {
            ...
            cnContact.phoneNumbers = contact.phoneNumbers.map {
                CNLabeledValue(label: nil, value: CNPhoneNumber(stringValue: $0))
            }
            cnContact.emailAddresses = contact.emails.map {
                CNLabeledValue(label: nil, value: $0 as NSString)
            }
